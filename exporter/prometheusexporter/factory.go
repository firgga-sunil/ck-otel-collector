...
// ========== ENHANCEMENT: Cleanup Methods on Wrapped Exporter ==========

// CleanByLabels removes metrics based on label filters
func (w *wrapMetricsExporter) CleanByLabels(filters map[string]string) int {
	return w.exporter.CleanByLabels(filters)
}

// CleanByMetricName removes metrics matching name pattern
func (w *wrapMetricsExporter) CleanByMetricName(namePattern string) int {
	return w.exporter.CleanByMetricName(namePattern)
}

// CleanExpired removes expired metrics
func (w *wrapMetricsExporter) CleanExpired() int {
	return w.exporter.CleanExpired()
}
